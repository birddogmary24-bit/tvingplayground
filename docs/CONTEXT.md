...
4. **미니게임**: 캐릭터 퀴즈 (4문제), 추천 룰렛, 명장면 매칭, 야옹이 키우기
5. **포인트 시스템**: 게임 보상 → 포인트 적립 → 유료 콘텐츠 구매
6. **로그인/구독**: 모달 기반 로그인, 3단계 구독권 (광고형/스탠다드/프리미엄)
7. **공개 일정**: 요일별 편성표 (오늘 하이라이트)
8. **히어로 배너**: 3개 콘텐츠 자동 회전 (4초 간격) + "클립 보러가기" → TVING 직접 이동
9. **상세 모달**: 콘텐츠 정보 + 클립/쇼츠 리스트 + "TVING 보기" 버튼
10. **3Pack 배너**: Disney+/TVING/Wavve 번들 프로모션 배너 (640x120 비율) → tving.com/list/theme/3pack 이동
